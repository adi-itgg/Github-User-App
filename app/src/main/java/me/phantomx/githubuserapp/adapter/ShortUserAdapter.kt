...
class ShortUserAdapter(
    private val activity: Activity,
    private var data: List<ShortUser> = listOf()
) : RecyclerView.Adapter<ShortUserAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) = with(holder.binding) {
        data[position].let {
            with(root.context) {
                ...
                name.text = it.login
                val score = resources.getString(R.string.score, it.score.toString())
                username.text = score

                root.setOnClickListener { _ ->
                    startActivity(
                        Intent(this, DetailsActivity::class.java)
                            .putExtra(ShortUser::class.simpleName, it),
    ...
    fun update(newData: List<ShortUser>) {
